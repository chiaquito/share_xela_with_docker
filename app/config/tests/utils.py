...
def setUp_cetegory_for_test():
    ...
    for ele in CATEGORY_CHOICE:
        value = ele[0]
        if Category.objects.filter(number=value).exists() is not True:
...
def create_user_for_test(userData):
    ...
    client = Client()
    client.post(CREATE_USER_URL, userData)
...
def create_item_data(
        category_obj, title=None, price=None,
        description=None, adm0=None, adm1=None, adm2=None
        ):
    """機能
...
def create_item_for_test(user_obj, item_data):
    ...
    client.post(CREATE_ITEM_URL, item_data)
...
def create_item_contact_for_test(comment_user_obj, data=None):
    ...
    client = Client()
    client.login(username=comment_user_obj.username, password="1234tweet")
...
def create_solicitud_for_test(item_obj, access_user_obj, solicitud_data):
    ...
    client = Client()
    client.login(username=access_user_obj.username, password="1234tweet")
...
def create_direct_message_for_test(solicitud_obj):
    ...
    # SOLICITUD_SELECT
    SOLICITUD_SELECT_URL = reverse(
        ViewName.SOLICITUD_SELECT,
        args=(solicitud_obj.id,),
        )
    item_obj = Item.objects.get(solicitudes__id=solicitud_obj.id)
    post_user_obj = item_obj.user
    client = Client()
    client.login(username=post_user_obj.username, password="1234tweet")
...
def create_item_for_android_test(key, data):
    ...
    client.post(CREATE_ITEM_URL, data)
