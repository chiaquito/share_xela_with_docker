...
from config.tests.utils import (
    ...
    create_user_for_android_test,
    create_item_for_android_test,
    create_item_data_for_android_test)
...
class ItemFavoriteAPIViewTest(TestCase):
    ...
    def setUp(self):
        ...
        fav_user = User.objects.create_user(
            username="fav_user",
            email="fav_test_user@gmail.com",
            password='12345')
        post_user = User.objects.create_user(
            username="post_user",
            email="test_post_user@gmail.com",
            password='12345')
        access_user = User.objects.create_user(
            username="access_user",
            email="test_access_user@gmail.com",
            password='12345')
        item_obj1 = Item.objects.create(
            user=post_user,
            id=1,
            title="テストアイテム１",
            description="説明です。",
    ...
    def test_should_remove_FavoriteObj_from_ItemObj_for_authenticated_user_with_favorite(self):
        # お気に入りを既にしている認証されたユーザーがFavボタンを押した時に
        # 当該UserオブジェクトがItem.favorite_usersから削除される *1
    ...
    def test_should_add_FavoriteObj_from_ItemObj_for_authenticated_user_with_favorite(self):
        # お気に入りをしていない認証されたユーザーがFavボタンを押した時には
        # 当該UserオブジェクトがItemオブジェクトのfavorite_usersに追加される *2
        item_obj1 = Item.objects.get(id=1)
        url = self.url.format(str(item_obj1.id))
...
class ItemDetailSerializerAPIViewGETTest(TestCase):
    ...
    済 ユーザーのアクセス状態によってBTN_CHOICEを変更する
    """
    ...
    def test_endpoint内のintに対応するidの記事データが返される(self):
        ...
        client = APIClient()
        client.credentials(HTTP_AUTHORIZATION='Token ' + self.post_user_key)
        ITEM_DETAIL_URL = reverse_lazy(
            "api:item_detail", args=(self.item_obj1.id,))
        response = client.get(ITEM_DETAIL_URL)

        # itemオブジェクトのidとResponseのItemオブジェクトのidが一致する内容を記述する
        self.assertEqual(
            response.data["item_obj_serializer"]["id"], self.item_obj1.id)

        client = APIClient()
        client.credentials(HTTP_AUTHORIZATION='Token ' + self.post_user_key)
        ITEM_DETAIL_URL = reverse_lazy(
            "api:item_detail", args=(self.item_obj2.id,))
        response = client.get(ITEM_DETAIL_URL)

        # itemオブジェクトのidとResponseのItemオブジェクトのidが一致する内容を記述する
        self.assertEqual(
            response.data["item_obj_serializer"]["id"], self.item_obj2.id)

        client = APIClient()
        client.credentials(HTTP_AUTHORIZATION='Token ' + self.post_user_key)
        ITEM_DETAIL_URL = reverse_lazy(
            "api:item_detail", args=(self.item_obj3.id,))
        response = client.get(ITEM_DETAIL_URL)
        print(response.data)

        # itemオブジェクトのidとResponseのItemオブジェクトのidが一致する内容を記述する
        self.assertEqual(
            response.data["item_obj_serializer"]["id"], self.item_obj3.id)
    ...
    def test_ItemContactオブジェクトがItemオブジェクトのプロパティとしてResponseで返される(self):
        ...
        response = client.get(ITEM_DETAIL_URL)
        self.assertTrue(
            "item_contacts" in response.data["item_obj_serializer"])
...
class ItemDetailSerializerAPIViewPATCHTest(TestCase):
    ...
    descriptionのみの変更の場合はdescriptionが変更される
    カテゴリーだけの変更の場合にはカテゴリーの変更が反映される
    radiusのみの変更の場合にはradiusが変更される
    ...
    def test_タイトルのみの変更の場合にはタイトルが変更される(self):
        ...
        response = client.get(ITEM_DETAIL_URL)
        self.assertEqual(
            response.data["item_obj_serializer"]["title"],
            item_obj1_before.title)        

        # 変更及び変更データ
        data = create_item_data_for_android_test(
            category_obj=self.item_obj1.category, title="changed")

        client = APIClient()
        client.credentials(HTTP_AUTHORIZATION='Token ' + self.post_user_key)
        response = client.patch(ITEM_DETAIL_URL, data)
        self.assertEqual(response.data["result"], "success")
        item_obj1_after = Item.objects.get(
            description=self.item_obj1.description)
    ...
    def test_descriptionのみの変更の場合はdescriptionが変更される(self):
        ...
        response = client.get(ITEM_DETAIL_URL)
        self.assertEqual(
            response.data["item_obj_serializer"]["description"],
            item_obj1_before.description
            )        

        # 変更及び変更データ *titleは入力しないと変わってしまう仕様なので入力している。変更はされていない。
        data = create_item_data_for_android_test(
            category_obj=self.item_obj1.category, 
            title=self.item_obj1.title,
            description="changed"
            )
        ...
        self.assertNotEqual(
            item_obj1_before.description, item_obj1_after.description)
    ...
    def test_カテゴリーだけの変更の場合にはカテゴリーの変更が反映される(self):
        ...
        data = create_item_data_for_android_test(
            category_obj=changedCategoryObj, title=self.item_obj1.title)        
        client = APIClient()
        client.credentials(HTTP_AUTHORIZATION='Token ' + self.post_user_key)
        response = client.patch(ITEM_DETAIL_URL, data)
        self.assertEqual(response.data["result"], "success")
        ...
        self.assertNotEqual(
            item_obj1_before.category.number, item_obj1_after.category.number)
    ...
    def test_radiusのみの変更の場合にはradiusが変更される(self):
        ...
        response = client.get(ITEM_DETAIL_URL)
        self.assertEqual(
            response.data["item_obj_serializer"]["radius"],
            item_obj1_before.radius
            )
